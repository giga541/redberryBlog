...
const SelectedBlog = () => {
  ...
  useEffect(() => {
    if (similarBlogs && blog) {
      setFilteredBlogs(
        similarBlogs.filter(
          b => b.id != blog.id && haveCommonCategories(b, blog)
        )
      );
    }
  }, [similarBlogs]);
    ...
    <div>
      ...
      {blog && (
        <div className={classes["blog_container"]}>
          ...
          <p className={classes["publish_date"]}>
        ...
        <div className={classes["similar-blogs-header"]}>
          ...
          <div className={classes["arrows"]}>
            <img
              ...
              onClick={() => setOffeset(Math.max(offset - 1, 0))}
            />
            <img
              src={ARROW_RIGHT}
              className={classes["arrow-right"]}
              onClick={() => setOffeset(Math.min(offset + 1, maxOffset))}
            />
          </div>
        </div>
